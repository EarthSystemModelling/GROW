...
from pyproj import CRS # imported version: 3.7.2
...
def calc_dd(config):
    """Calculates global drainage density (sum of river lengths/ area of basin)
    with HydroBASINS and HydroRIVERS.
    ...
    # calculate "drainage density" per basin and correct for area distortion
...
def merge_raster_static(df, rasterfile, col_name=None, transform=False):
    """In this function, raster values of a static target variable are extracted at
    the well locations and merged to the well attributes table.
    ...
    # add list as column to our dataframe
    df[col_name] = pd.Series(values)

def merge_vector_point(vector, df, columns):
    """In this function, polygon values of a static target variable are extracted at the well
    locations and merged to the well attribute table via spatial join.
    ...
    # they might be duplicates when a location is at the border of two polygons
    joined_gdf = joined_gdf.drop_duplicates(subset="GROW_ID")
...
def extract_val(config, row, raster, bandname, latname, lonname, col_name, ts):
    """In this function, raster values of a time-varying target variable are extracted at
    a specific well location. The extracted values are appended to an output file.
    ...
    """
    if bandname=="NDVI":
        if (raster["latitude"].to_index().is_unique==False) | (raster["longitude"].to_index().is_unique==False):
            print(ts)
            return
    ...
    ts_df = pd.DataFrame(
        {
            "GROW_ID": row["GROW_ID"],
            "Time": ts or extract.time.values,
            "Interval": row["interval"],
            "Value": extract[bandname].values,
        }
    )

    # cut time - to save storage, only time stamps that actually occur
    # in the groundwater time series are kept in the dataframe
    # substract one year from "starting_date" so that monthly or yearly time series
    # which do not start at the first day of the year/month are still included
    ...
    # which do not end with the last day of the year/month are still included
...
def merge_raster_transient(
    ...
    for file in rasterfiles:
        ...
        if ndvi:
            # time cannot be decoded automatically
            ...
            # deriving time from file name, because time cannot be decoded automatically
            ts = datetime(int(file[-27:-23]), int(file[-23:-21]), int(file[-21:-19]))
        elif isimip:
            # time cannot be decoded automatically
            raster_nc = xr.open_dataset(file, decode_times=False)
            # time dimension is always the same for ISIMIP 3a data, 1901 - 2021 and has yearly resolution
            ...
            if era5:
                ...
                # time dimension must be renamed because it is addressed with the name
                # "time" for the rest of the function
...
def aggregate_merge(config, df, imp_name, i, exp_name, daily=False, yearly=False):
    """In this function, the dataframes of the extracted time-varying target variables are imported,
    aggregated to the temporal resolution of the groundwater data
    ...
    # set negative snow_depth to 0
    # and add days with snow cover per well and year as attribute to table
    if imp_name == "snow_depth":
        raster.Value[raster.Value < 0] = 0
        # calculate average days of snow days per year
        def num_snow_days(df):
            if df.Value.notna().all():
                number_of_days = len(df[(df.Value != 0) & (df.Value.isna() == False)])
                return number_of_days
            else:
                return None
        # aggregate data to a yearly resolution and calculate the average days with snow
        # cover per year within the aggregate function
        raster["year"] = raster["time"].dt.year.astype("int")
        yearly_aggregate = raster.groupby(["GROW_ID","year"],as_index=False).apply(num_snow_days)
        # merge to not aggregated table
        raster = raster.merge(yearly_aggregate, on=["GROW_ID","year"])
        raster.drop(columns="year",inplace=True)
        raster.columns = ["GROW_ID","time","interval","Value","days_with_snow_cover_days_year-1"]

    # create datetime column which shall be used to aggregate and merge the data;
    # datetime format depends on temporal resolution of groundwater data
    raster["merge"] = None
    ...
    # aggregation to groundwater time series resolution by means
    raster_agg = raster.groupby(["merge", "interval", "GROW_ID"]).mean()
    raster_agg.reset_index(inplace=True)
    raster_agg.drop(["interval", "time"], axis=1, inplace=True)
    ...
    if yearly:
        ...
        df_merged = pd.merge(
            ...
            raster_agg,
            how="outer",
            left_on=["year", "GROW_ID"],
            right_on=["merge", "GROW_ID"],
        )
    elif daily:
        ...
        df_merged = pd.merge(
            ...
            raster_agg,
            how="outer",
            left_on=["date", "GROW_ID"],
            right_on=["merge", "GROW_ID"],
        )
    else:
        # date2: here the daily data was trimmed to a monthly resolution
        # so that Earth system variables with monthly resolution can be merged to daily data
        df_merged = pd.merge(
            df,
            raster_agg,
            how="outer",
            left_on=["date2", "GROW_ID"],
            right_on=["merge", "GROW_ID"],
        )
