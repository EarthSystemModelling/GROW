...
attributes = pd.read_csv(
    "/mnt/storage/grow/final_grow/grow_attributes.csv", sep=";"
)  # table in csv-format
timeseries = pd.read_parquet(
    "/mnt/storage/grow/final_grow/grow_timeseries.parquet"
...
]  # subset attribute table by data flag columns
ts_bra = timeseries[
    timeseries.GROW_ID.isin(attr_bra_monthly.GROW_ID)
...
ts_bra_monthly = ts_bra.groupby(
    ["GROW_ID", "month"], as_index=False
).mean()  # aggregate all numeric variables per ID and month
